...
const NUM = '0-9零〇○一壹二贰两俩三付四五六七八九十';
...
/** 动作词（通道 B） */
export const ACTION_RE = /(给我|跟我|和我|叫我|喊我|催我|通知我|发(?:条|个|一条|一个)?(?:消息|信息|微信|短信|语音|红包)|打电话|视频|背单词|打卡|吃药|喝水|喝药|起床|睡觉|开会|上课|签到|交(?:作业|电费|水费|房租|钱|费)|买|做饭|煮|洗洗|洗衣|下单|充电|提交|复习|锻炼|跑步|运动|遁狗|浇花|倒垃圾|接娃|送娃|预约|体检|繳费|还款|回电|回复|拿快递|取快递|寄快递|看球|记账|学习|休息)/;
...
/** 行首剩下的赘余动词/语气词，剔掉后才是真正的任务名 */
const LEAD_FILLER_RE = /^(?:要去|帮我|给我|跟我|和我|顺便|然后|记得|再|就|把|去|要)+/;
const EDGE_PUNCT_LEAD_RE = /^[的了吧呀啊呢吗，,。.、：:；;~!！?？\s]+/;
const EDGE_PUNCT_TAIL_RE = /[，,。.、：:；;~!！?？\s]+$/;

/** 从原句中抽取任务标题（去掉时间词、触发词与行首赘余词） */
export function extractTaskTitle(message, fallback = '你交给我的事') {
  ...
  s = s.replace(EDGE_PUNCT_LEAD_RE, '');
  s = s.replace(LEAD_FILLER_RE, '');
  s = s.replace(EDGE_PUNCT_LEAD_RE, '');
  s = s.replace(EDGE_PUNCT_TAIL_RE, '');
  s = s.trim();
