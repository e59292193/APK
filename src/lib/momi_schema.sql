...
CREATE TABLE IF NOT EXISTS public.momi_assistant_messages (
  ...
  sender varchar NOT NULL, -- 'momo' | '苞米' | 'momi'
...
CREATE TABLE IF NOT EXISTS public.momi_memory (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  couple_id varchar NOT NULL DEFAULT 'momo_and_baomi',
  user_id varchar NOT NULL, -- 'momo' | '苞米' | 'both'
  memory_type varchar NOT NULL, -- 'habit' | 'personality' | 'preference' | 'milestone'
  content text NOT NULL,
...
-- 6. Storage Bucket: avatars (公开头像存储桶) ───────────────
